...
ctgLocs2refLocs <- function(K,aln) {
  ...
  mcols(h)$genomic <- GRanges(seqnames(aln[subjectHits(h)]),pmapFromAlignments(mcols(h)$qcov,aln[subjectHits(h)]))
...
#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#
variant.table <- function(bamKmer,bamCtg,gff) {
  #-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#
  ...
  #-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#
  # Annotate genomic positions with closest gene
  #-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#
  K$coveredGenomicClusters <- local({
    # find genomic clusters
    G <- unlist(K$coveredGenomicRegions)
    Gclusters <- reduce(G,with.revmap=TRUE)
    
    # annotate clusters with closest gene
    nn <- nearest(Gclusters,gff)
    Gclusters$nearest_gene_N315id <- gff$N315_gene_id[nn]
    Gclusters$nearest_gene_tag <- gff$locus_tag[nn]
    Gclusters$nearest_gene_distance[!is.na(nn)] <- distance(Gclusters,gff[nn[!is.na(nn)]])
    names(Gclusters) <- paste0(
      as.character(Gclusters),":",
      Gclusters$nearest_gene_tag,":",
      Gclusters$nearest_gene_distance,":",
      Gclusters$nearest_gene_N315id
    )
    Gclusters$rk <- seq(length(Gclusters))
    
    relist(Gclusters[findOverlaps(G,Gclusters,select="first")],K$coveredGenomicRegions)
  })
  K <- K[order(min(relist(unlist(K$coveredGenomicClusters)$rk,K$coveredGenomicClusters)))]
...
plot.variants.stats <- function(K) {
  K$alnSeqname <- drop(CharacterList(unique(seqnames(K$coveredGenomicClusters))))
  K$alnSeqname[is.na(K$alnSeqname)] <- "unmapped"
  K$minClosestGeneDistance <- min(with(stack(K$coveredGenomicClusters),split(nearest_gene_distance,name)))
  ggplot(as.data.frame(K),aes(x=maxKmerCoverage,y=avgKmerCount)) + 
    facet_wrap(~name) + 
    geom_point(aes(size=width,color=alnSeqname,shape=minClosestGeneDistance==0)) + 
...
export.variant.tsv <- function(K,tsv.file) {
  K$coveredGenomicRegions <- unstrsplit(relist(with(unlist(K$coveredGenomicRegions),paste0(seqnames,":",start,"-",end,":",mapq,":",cigar)),K$coveredGenomicRegions),",")
  K$coveredGenomicClusters <- unstrsplit(relist(names(unlist(K$coveredGenomicClusters)),K$coveredGenomicClusters),",")
  K <- as.data.frame(K)
  K$contig <- K$strand <- NULL
  K$chr <- unstrsplit(CharacterList(unique(seqnames(vartbl$coveredGenomicRegions))),";")
  
  write.table(K,file=tsv.file,row.names=FALSE,sep="\t")
}
export.variant.tsv(vartbl,"out/variants.tsv")
...
function() {
      #-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#
      # exporting variants positions
      #-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#
      files <- data.frame(stringsAsFactors = FALSE,
          kmermap = list.files("data",".kmermap.bam$",recursive=TRUE,full.names=TRUE),
          ctgmap = list.files("data",".bwamem.bam$",recursive=TRUE,full.names=TRUE),
          asqg = list.files("data",".asqg.gz$",recursive=TRUE,full.names=TRUE)
      )
      files$library <- basename(dirname(files$asqg))
      files <- files[files$library!="SupSVK35",]
      
      gff <- import.gff("data/SA564_features.gff3.gz",feature.type="gene")
      vartbl <- mapply(variant.table,files$kmermap,files$ctgmap,MoreArgs=list(gff=gff))
      vartbl <- GRangesList(vartbl)
      names(vartbl) <- files$library
      vartbl <- stack(vartbl)
      
      
      p <- plot.variants.stats(vartbl)
      ggsave("out/variants.pdf",p,width=12,height=10)
      
      mapply(
        function(asqg,ctg,lib)
          visAssemblyGraph(asqg,ctg,vartbl$contig[as.logical(vartbl$name==lib)]) %>% 
          visSave(paste0(lib,".html")),
        files$asqg,files$ctgmap,files$library
      )
      
      
      #gff
      export.gff(unique(unlist(vartbl$coveredGenomicClusters)),"out/genomicVariantsClusters.gff")
      ...
      vartbl$chr <- unstrsplit(CharacterList(unique(seqnames(vartbl$coveredGenomicRegions))),";")
      V <- vartbl[vartbl$chr != "CP010891.1",]
}
